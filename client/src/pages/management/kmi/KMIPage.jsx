...
export default function KMIPage() {
  ...
  const [form, setForm] = useState({ description: '', start_date: '', end_date: '', status: 'Pending', notes: '' })
  const [error, setError] = useState('')

  const load = async () => {
    try {
      const res = await api.get('/kmi')
      setRows(res.data)
    } catch (e) {
      setRows([])
    }
  ...
  const onClose = () => {
    ...
    setEditingId(null)
    setForm({ description: '', start_date: '', end_date: '', status: 'Pending', notes: '' })
    setError('')
  }
  ...
  const submit = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/kmi/${editingId}`, form)
      } else {
        await api.post('/kmi', form)
      }
      onClose(); load()
    } catch (e) {
  ...
  const remove = async (id) => {
    if (!confirm('Delete this KMI?')) return
    try { await api.delete(`/kmi/${id}`); load() } catch (e) { /* ignore */ }
  }
    ...
    <div className="min-h-screen px-4 py-12 bg-gradient-to-br from-gray-50 to-gray-100 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 text-center">
          ...
          <p className="text-lg text-gray-600">Key Milestone Indicators - Define and track strategic milestones</p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.length === 0 ? (
                  <tr><td colSpan="7" className="px-6 py-12 text-center text-gray-500">No KMIs found</td></tr>
                ) : (
                  paginated.map((r, idx) => (
                    <tr key={r.id}>
              ...
              <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                <div className="px-6 py-4 bg-gradient-to-r from-indigo-600 to-purple-600">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-medium leading-6 text-white">{editingId ? 'Edit KMI' : 'Add KMI'}</h3>
                ...
                <div className="px-6 py-5 bg-white">
                  ...
                  <form className="space-y-5" onSubmit={submit}>
                    <div>
                      ...
                      <textarea rows="2" value={form.description} onChange={e=>setForm({ ...form, description: e.target.value })} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="KMI description" required />
                    </div>
