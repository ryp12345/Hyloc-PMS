...
const initialForm = { qual_name: '', qual_shortname: '', status: 'active' };
...
export default function QualificationsPage() {
  ...
  const load = async () => {
    try { const res = await getQualifications(); setRows(res.data || []); }
    catch { setRows([]); }
  };
  useEffect(() => { load(); }, []);
  ...
  const submit = async (e) => {
    ...
    try {
      const payload = { ...form };
      if (editingId) await updateQualification(editingId, payload);
  ...
  const toggleStatus = async (row) => {
    ...
    if (!confirm(`Mark this qualification as ${newStatus}?`)) return;
    try { 
      await updateQualification(row.id, { 
        qual_name: row.qual_name,
        qual_shortname: row.qual_shortname,
        status: newStatus 
      }); 
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter(r => (
      r.qual_name?.toLowerCase().includes(q) ||
      r.qual_shortname?.toLowerCase().includes(q) ||
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col items-start justify-between gap-4 mb-6 sm:flex-row sm:items-center">
          <div className="relative w-full sm:w-72">
            <input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search qualifications..." className="w-full py-2 pl-10 pr-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.length === 0 ? (
                  ...
                  paginated.map((q, idx) => (
                    <tr key={q.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-indigo-50 transition-colors duration-150`}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{(page - 1) * PAGE_SIZE + idx + 1}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{q.qual_name}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{q.qual_shortname}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm"><span className={`px-3 py-1 text-xs font-medium rounded-full ${q.status==='active'?'bg-green-100 text-green-800':'bg-gray-100 text-gray-800'}`}>{q.status}</span></td>
                      <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div className="flex items-center justify-center space-x-2">
                          ...
                          <button
                            ...
                            className={`p-2 text-white transition-colors duration-200 rounded-lg ${q.status === 'active' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}
                            title={q.status === 'active' ? 'Mark as Inactive' : 'Mark as Active'}
                          >
                            {q.status === 'active' ? (
                             <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <form className="space-y-5" onSubmit={submit}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div>
                        ...
                        <input value={form.qual_name} onChange={e=>setForm({ ...form, qual_name: e.target.value })} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Bachelor of Science" required />
                      </div>
                      <div>
                        <label className="block mb-2 text-sm font-medium text-gray-700">Short Name</label>
                        <input value={form.qual_shortname} onChange={e=>setForm({ ...form, qual_shortname: e.target.value })} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="B.Sc" required />
                      </div>
                      {editingId && (
                        <div>
