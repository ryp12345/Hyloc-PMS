...
export default function DesignationsPage() {
  ...
  const load = async () => {
    try { const res = await getDesignations(); setRows(res.data || []); }
    catch { setRows([]); }
  };
  useEffect(() => { load(); }, []);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col items-start justify-between gap-4 mb-6 sm:flex-row sm:items-center">
          <div className="relative w-full sm:w-72">
            <input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search designations..." className="w-full py-2 pl-10 pr-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                ...
                {filtered.length === 0 ? (
                  ...
                  paginated.map((d, idx) => (
                    <tr key={d.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm"><span className={`px-2 py-1 text-xs rounded border ${d.status==='active'?'bg-green-50 text-green-700 border-green-100':'bg-gray-50 text-gray-700 border-gray-200'}`}>{d.status}</span></td>
