...
export default function KAIPage() {
  ...
  const [form, setForm] = useState({ kpi_id: '', description: '', start_date: '', end_date: '', status: 'Pending', notes: '' });
  const [error, setError] = useState('');

  const load = async () => {
    const [kai, kpi] = await Promise.all([api.get('/kai'), api.get('/kpi')]);
    setRows(kai.data);
    setKpis(kpi.data);
  };
  useEffect(() => { load(); }, []);
  ...
  const onClose = () => {
    ...
    setEditingId(null);
    setForm({ kpi_id: '', description: '', start_date: '', end_date: '', status: 'Pending', notes: '' });
    setError('');
  };
  ...
  const submit = async (e) => {
    ...
    try {
      const payload = { ...form, kpi_id: form.kpi_id ? parseInt(form.kpi_id) : null };
      if (editingId) await api.put(`/kai/${editingId}`, payload);
      else await api.post('/kai', payload);
      onClose(); load();
    } catch (e) { setError(e.response?.data?.message || 'Failed to save'); }
  };

  const remove = async (id) => {
    if (!confirm('Delete this KAI?')) return;
    try { await api.delete(`/kai/${id}`); load(); } catch {}
  };
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter(r => (
      ...
      (kpis.find(k=>k.id===r.kpi_id)?.description?.toLowerCase().includes(q)) ||
    ...
    <div className="min-h-screen px-4 py-12 bg-gradient-to-br from-gray-50 to-gray-100 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 text-center">
          <h1 className="mb-2 text-4xl font-extrabold text-gray-900">KAI Management</h1>
          <p className="text-lg text-gray-600">Create, update and manage Key Action Items</p>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">KPI</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.length === 0 ? (
                  <tr><td colSpan="8" className="px-6 py-12 text-center text-gray-500">No KAIs found</td></tr>
                ) : (
                  paginated.map((r, idx) => (
                    <tr key={r.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{kpis.find(k=>k.id===r.kpi_id)?.description || '-'}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm"><span className={`px-2 py-1 text-xs rounded border ${getStatusColor(r.status)}`}>{r.status}</span></td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{r.notes || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button onClick={()=>openEdit(r)} className="mr-3 text-indigo-600 hover:text-indigo-900">Edit</button>
                        <button onClick={()=>remove(r.id)} className="text-red-600 hover:text-red-900">Delete</button>
                      </td>
                    </tr>
                  ))
                )}
              ...
              <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                <div className="px-6 py-4 bg-gradient-to-r from-indigo-600 to-purple-600">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-medium leading-6 text-white">{editingId ? 'Edit KAI' : 'Add KAI'}</h3>
                ...
                <div className="px-6 py-5 bg-white">
                  ...
                  <form className="space-y-5" onSubmit={submit}>
                    <div>
                      <label className="block mb-2 text-sm font-medium text-gray-700">Select KPI</label>
                      <select className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" value={form.kpi_id} onChange={e=>setForm({...form, kpi_id:e.target.value})} required>
                        <option value="">-- Select KPI --</option>
                        {kpis.map(k => <option key={k.id} value={k.id}>{k.description}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block mb-2 text-sm font-medium text-gray-700">Description</label>
                      <input className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="KAI description" value={form.description} onChange={e=>setForm({...form, description:e.target.value})} required />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div>
                        <label className="block mb-2 text-sm font-medium text-gray-700">Start Date</label>
                        <input type="date" className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" value={form.start_date} onChange={e=>setForm({...form, start_date:e.target.value})} required />
                      </div>
                      <div>
